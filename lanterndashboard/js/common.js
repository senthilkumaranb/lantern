...
function accountType (response){
  ...
  PolicyApi.readPolicy();
  console.log(response);
  
  //this is hardcoded, need to be made dynamic
  if (response=='PMR') {
    tree = new Treant( chart_config );
  } else if (response=='Deal') {
    tree = new Treant( deal_chart_config );
  }
  ...
  var res=response;
   ...
   var context;
      ...
      if (latestResponse) {
        ...
        //senthil: to upate the KB graph
        if ($('#runmode').is(":checked"))
        {
          // it is checked
          if(context.hasOwnProperty('runmode')) {
            context.runmode="auto";
            console.log (context.runmode);
          }
          
        }
        //updateKBgraph("/" + context.kbgraph + ".json");
        drawDT(context.kbgraph);
        
      }

      console.log("common context " + JSON.stringify(context));

      console.log (context.kbgraph);
      console.log ("tres---" + $('#tresstate').val());

      context.tres = parseInt($('#tresstate').val());
      console.log("context res--" + context.tres);

      if (parseInt(context.mail)==1){
